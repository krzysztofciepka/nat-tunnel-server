  ...
  _createServer(clientId) {
    return new Promise((resolve, reject) => {
      ...
      server.on('connection', (socket) => {
        debug(`Connection established for client ${clientId} on port ${socket.address().port}`);
        ...
        socket.on('close', (hasError) => {
          debug(`Connection closed. Client: ${clientId} port: ${socket.address().port} hasError: ${hasError}`);
        });
      });
      ...
      server.on('close', () => {
        debug('Server closed on port: ', server.address().port);
        this._removeClient(clientId);
      });
